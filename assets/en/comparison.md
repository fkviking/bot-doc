...
|Data Access Speed| Formula code is executed whenever price calculation by main algorithm is invoked. Accordingly, formulas are recalculated immediately after any order book update triggers the main pricing calculation. No rate limits apply to data accessible within formulas|Requires sending explicit requests to get or modify data; subject to [rate limits](api.md#api.rate_limits) . When subscribed to data updates, the server sends changes in batches at a [specific frequency](api.md#api.updates_rate) , not on every change|
