...
## Индикаторы, математическая статистика и коллекции<Anchor :ids="['indicators-docs']"/>
...
#### Сохранение значений индикаторов и коллекций между перезапусками робота
...
##### Просмотр и удаление значений индикаторов и коллекций, размещенных в `shared memory`
...
#### Дек на основе кольцевого буффера (`ring_deque`) <Anchor :ids="['ring-deque']"/>
...
## Примеры работы с индикаторами и коллекциями, размещенными в `shared memory`
...
---
Создать дек ([`ring_deque`](c-api.html#ring-deque)) в `shared memory`, загрузив данные из `shared memory`, в качестве элемента дека использовать заданную структуру:
```C
struct point
{
  int x = 0;
