# 10. Формулы на языке программирования C++
...
## 10.1. Редактор формул

Редактировать формулы на C++ вы можете во вкладке `Formulas` в окне настроек портфеля, которое открывается при нажатии на иконку <img src="/bot-doc/docs/00-img/icons/settings_black.svg" width="16" height="16"/> :
...
## 10.2. Базовые принципы написания формул
...
- Максимальная разрешенная длина каждого из полей формул в портфеле в данный момент составляет `12000` байт (это значение можно получить в [шаблоне портфеля](api.md#get_template_id)).
...
возможность небольшого расширения алгоритма работы робота пользователем, если же вам необходимо что-то намного более значительное, то для этих целей рекомендуется использовать [WebSocket API](api.md#api);
...
## 10.3. Рекомендации по фильтрации биржевых данных
...
## 10.4. Базовые принципы отладки формул
...
## 10.5. Доступ к биржевым данным по финансовым инструментам
...
| double bid()<a id="bid-offer-check"></a>                    | лучшая цена на покупку (`0` если цены нет, рекомендуется проверять вместе с `amount_bid`) |
...
| int trading_status()<a id="trading-status-check"></a>            | статус торгуемости финансового инструмента на бирже (битовая маска, возможные взведенные биты [TRADING_CAN_PLACE](#__trading_can_place__) и [TRADING_CAN_CANCEL](#__trading_can_cancel__), [пример](#__sec_status_check__)) |
| int conn_online()<a id="market-data-check"></a>               | статус активности маркет-дата подключения в роботе (битовая маска, возможные взведенные биты [MARKET_DATA_BESTS_ONLINE](#__market_data_bests_online__) и [MARKET_DATA_OB_ONLINE](#__market_data_ob_online__), [пример](#__sec_status_check__)) |
...
| Метод                  | Описание                                                                                                                                                         |
|------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| bool is_available()<a id="order-book-check"></a>    | доступен ли в данный момент для данного финансового инструмента стакан                                                                                                             |
...
## 10.6. Доступ и изменение полей инструмента портфеля
...
## 10.7. Доступ и изменение полей портфеля
...
| deal_item deal() <a id="__last_deal__"></a>  | получить последнюю сделку по финансовым инструментам портфеля, если сделок не было, то `amount` и `price` будут равны нулю |
...
Для того чтобы иметь возможность обращаться по индексу к "пользовательским полям" есть массивы:<a id="user-fields"></a>
```C
...
## 10.8. Доступ и изменение позиций транзакционного подключения
...
| Метод                               | Описание                                                                                                                               |
|-------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| sec_item sec(const std::string& s)  | получить информацию о позиции инструмента на бирже (в качестве `s` необходимо использовать [SecCode](params-description.md#pos-sec-code))  |
| coin_item sec(const std::string& s) | получить информацию о балансе валюты на бирже (в качестве `s` необходимо использовать [Currency](params-description.md#pos-currency))       |
...
## 10.9. Дополнительные классы, структуры, функции и константы
...
| <a id="__trading_can_place__"></a>TRADING_CAN_PLACE | int         | 1, разрешено выставления заявок               |
| <a id="__trading_can_cancel__"></a>TRADING_CAN_CANCEL | int         | 2, разрешено снятие заявок                   |
| MARKET_DATA_OFFLINE   | int         | 0, маркет-дата подключение для данного финансового инструмента оффлайн |
| <a id="__market_data_bests_online__"></a>MARKET_DATA_BESTS_ONLINE | int         | 1, для данного финансового инструмента маркет-дата подключение с лучшими ценами на покупку/продажу онлайн|
| <a id="__market_data_ob_online__"></a>MARKET_DATA_OB_ONLINE | int         | 2, для данного финансового инструмента маркет-дата подключение со стаканами онлайн |
| NAME           | std::string | имя текущего портфеля                            |
| <a id="__null_value__"></a>NULL_VALUE     | long long   | Пустое значение для числовых полей, равное `-(1 << 53)` |
...
| void log_info(const std::string& msg) <a id="cpp-debug-functions"></a>                    | отправить сообщение с уровнем INFO в лог      |
...
| void log_show(const std::string& msg) <a id="cpp-log-show"></a>                      | отправить сообщение с уровнем INFO в лог, данное сообщение будет отображено в виджете [`Robot logs`](interface.md#robot_logs)      |
| <a id="__tgr_notify__"></a> bool tgr_notify(int slot, const std::string& msg, int timeout) | отправить сообщение с уровнем NOTIFICATION в лог и уведомление в telegram, отправлять не чаще, чем `timeout` секунд (число из отрезка `[10, 2000000000]`), `slot` - идентификатор слота уведомления (число из отрезка `[0, 4]`), для которого будет отсчитываться `timeout` |

### 10.9.3. Структуры

Структура `day_time` (время дня в часах, минутах, секундах):<a id="day_time"></a>
...
Класс `timer` (отмеряет отрезки времени не менее заданного в наносекундах): <a id="timers"></a>
...
### 10.9.5. Функции для работы с опционами

#### 10.9.5.1. _“Умные” функции_
...
## 10.10. Индикаторы, математическая статистика и коллекции

### 10.10.1. Общие положения

#### 10.10.1.1. Особенности объектов для расчета значений индикаторов

Для расчета индикаторов используются объекты одноименных классов. Т.к. объекты создаются внутри функции, соответственно, чтобы все объекты каждый раз не создавались заново и инициализировались только один раз, их предполагается создавать как статические (`static`) переменные. Если для расчета значения индикатора нужна история, а ее в данный момент не хватает (т.е. количество значений в истории меньше, чем `length`), то в качестве значения `length` индикаторов будет использоваться текущее количество элементов в накопленной истории.
...
#### 10.10.1.2. Сохранение значений индикаторов и коллекций между перезапусками робота
...
Описанные в данном разделе коллекции, так же как и индикаторы, создаются напрямую в `shared memory`

Данное решение позволяет безболезненно переживать перезапуск робота, т.к. все данные очередей индикаторов и коллекций находятся в `shared memory`. 
При выключении робота с этими данными ничего не произойдет: когда вы запустите робота и он вызовет конструктор индикатора или коллекции при вызове формулы, данные подгрузятся обратно из соответствующей области памяти.

##### 10.10.1.2.1. Особенности реализации

<a id="shared-memory"></a>
1. Для того чтобы размещать очереди индикаторов и коллекции в `shared memory` в конструкторах всех индикаторов и коллекций есть следующие необязательные параметры:
    |Тип|Имя|Описание|
    |---|---|---|---|
    |`const std::string&`|`key`| Уникальное имя индикатора/коллекции. Если не указано или указано пустым, очереди будут размещены в оперативной памяти, но не в `shared memory`. Для коллекций нельзя не указать или указать пустым. Длина не может быть больше, чем `32` символа, может состоять только из букв латинского алфавита (строчных и заглавных), цифр и символа подчеркивания, иначе будет выброшено исключение `std::invalid_argument` |
    ...
    |`bool`|`save`| Сохранять данные в `shared memory`. По умолчанию `true` для "боевого" режима и `false` при тестировании формул (`#ifdef DEBUG`). Если `true` и `key` не пусто, то очередь/коллекция будет размещена в `shared memory` |
   
    По умолчанию:  в "боевом" режиме робот подгружает очереди индикаторов и коллекции из заданной области `shared memory` и пользуется этими данными, кроме того новые данные сохраняются в `shared memory`. При этом в режиме тестирования формул (`#ifdef DEBUG`) данные подгружаются из `shared memory`, т.е. при какой-либо правке формулы, ее можно протестировать на уже накопленных данных, а не на пустой очереди/коллекции. По умолчанию новые данные, полученные при тестировании, не сохраняются обратно в `shared memory`, чтобы не испортить "боевую" очередь/коллекцию конкурентной записью.
   
   **Важно!** Это только поведение по умолчанию, задавая значения параметров `save` и `load` данное поведение можно поменять.
   
2. Реализовано автоматическое удаление старых неиспользуемых индикаторов/коллекций, размещенных в `shared memory`. Данные из `shared memory` удаляются:
   ...
   - НЕ используемые сейчас в портфелях коллекции - после `72` часов без обновлений и доступа к этим коллекциям.
3. Реализована возможность просмотра текущего списка индикаторов, чьи очереди размещены в `shared memory`, и коллекций а также возможность удаления неиспользуемых индикаторов/коллекций из `shared memory`.


##### 10.10.1.2.2. Ограничения

1. Перезапуск переживут только данные из очередей индикаторов и коллекций в `shared memory`. Если какие-либо данные хранились в каких либо структурах данных в оперативной памяти (например, `std::vector` или `std::map`, или что-то еще), то эти данные сохранены НЕ будут!
    В связи с этим настоятельно рекомендуется пользоваться готовыми индикаторами (а не писать их самим) и, если важно не терять данные при перезапуске робота, сохранять все важные данные в "пользовательские поля" и коллекции в `shared memory`.
...
3. В рамках одного робота, одного портфеля и одного и того же сохраняемого в `shared memory` типа индикатора/коллекции, имя индикатора/коллекции нельзя использовать дважды (чтобы несколько индикаторов/коллекций не писали конкурентно в одну и ту же область памяти).
4. Для каждого робота есть лимит на число создаваемых в `shared memory` индикаторов/коллекций (по умолчания он равен `400`, при превышении лимита будет выброшено исключение `std::invalid_argument`).
5. **Для коллекций в качестве строкового имени типа используется `rq` + `<РАЗМЕР_ШАБЛОННОГО_ТИПА>`, т.е., например, для типа данных `double`, чей размер `8` байт, тип будет `rq8`. Т.к. разные типы данных могут иметь одинаковый размер, то возможна ситуация когда данные были
    сохранены для одного типа, а потом пользователь пытается загрузить эти данные для другого типа (оставив старое значение параметра `key`, но при этом поменяв сам шаблонный тип), в результате скорее всего будет вычитан "мусор". Следить за тем что было сохранено, и что потом
    было загружено - ответственность пользователя!**


##### 10.10.1.2.3. Просмотр и удаление значений индикаторов и коллекций, размещенных в `shared memory`

| Функция                                                                                            | Описание                                      |
|----------------------------------------------------------------------------------------------------|-----------------------------------------------|
| std::vector<indicator_info> get_indicators()                                                       | получить список сохраненных в `shared memory` индикаторов/коллекций робота |
| std::pair<bool, indicator_info> get_indicator(const std::string& pname, const std::string& type, const std::string& iname) | получить сохраненный в `shared memory` индикатор/коллекцию робота по имени портфеля `pname`, типу индикатора/коллекции `type` и имени индикатора/коллекции `iname`, если нидикатор/коллекция не будет найден, первый элемент пары будет равен `false`, а второй будет содержать "мусор" |
| bool del_indicator(const std::string& pname, const std::string& type, const std::string& iname)    | удалить сохраненный в `shared memory` неиспользуемый индикатор/коллекцию робота по имени портфеля `pname`, типу индикатора/коллекции `type` и имени индикатора/коллекции `iname`, вернуть `true` если такого индикатора/коллекции больше не существует |
| bool del_indicator(const indicator_info& ii)                                                       | удалить сохраненный в `shared memory` неиспользуемый индикатор/коллекцию робота, вернуть `true` если такого индикатора/коллекции больше не существует |

Поля `indicator_info`:

| Название   | Тип       | Описание    |
|------------|-----------|-------------|
| name       | std::string    | имя индикатора/коллекции        |
| type       | std::string    | тип индикатора/колелкции       |
| portf      | std::string    | имя портфеля к которому относится индикатор/коллекция        |
| long_name  | std::string    | полное имя индикатора/коллекции |
| bool       | used    | используется ли индикатор/коллекция сейчас |
| length     | size_t    | длина индикатора или вместимость коллекции        |
| timeframe  | size_t    | тайм-фрейм индикатора, неприменимо для индикатора       |
| size       | size_t    | текущий размер списка элементов, по которым считается индикатор, или вместимость коллекции       |
| long long  | min_dt    | минимальные дата/время элемента, по которому считается индикатор, в формате `epoch`, неприменимо для индикатора|
| long long  | max_dt    | максимальные дата/время элемента, по которому считается индикатор, в формате `epoch`, неприменимо для индикатора|
| long long  | last_updated    | дата/время последнего обновления индикатора или обновления/доступа к коллекции в формате `epoch`|
...
[_Примеры работы с индикаторами и коллекциями, размещенными в `shared memory`._](#__ExampleInd__)

#### 10.10.1.3. `TradingDays` и `schedule` индикатора
...
#### 10.10.1.4. Объект `indicators::interval`
...
#### 10.10.1.5. Объект `indicators::schedule`
...
#### 10.10.2.1. Простое скользящее среднее (`SMA`)
...
#### 10.10.2.2. Экспоненциальное скользящее среднее (`EMA`)
...
#### 10.10.2.3. Максимум на отрезке (`Max`)
...
#### 10.10.2.4. Минимум на отрезке (`Min`)
...
#### 10.10.2.6. Среднеквадратическое отклонение (`StdDev`)
...
#### 10.10.2.7. Линии Боллинджера (`BB`)
...
#### 10.10.2.8. Индекс относительной силы (`RSI`)
...
#### 10.10.3.1. Дек на основе кольцевого буффера (`ring_deque`)

Дек (двусторонняя очередь) размещает данные в кольцевом буффере, который в свою очередь размещается в `shared memory`. Т.к. данные размещаются в `shared memory`, на тип этих данных накладываются следующие ограничения:
1. тип должен быть тривиально копируемым;
2. тип должен быть стандартно-компонуемым;
3. тип не может быть указателем или умным указателем, если это структура, то ее поля тоже не могут быть указателями и умными указателями.

Под данные всегда выделяется буффер размером `161 600` байт, соответственно, максимальное количество элементов, которые можно положить в дек, зависит от размера элемента

Конструкторы `ring_deque`:

| Конструктор                                         | Описание                                                                  |
|-----------------------------------------------------|---------------------------------------------------------------------------|
| ring_deque&lt;T&gt;(const std::string& key, bool load = true, bool save = ...) | создать объект, хранящий объекты типа `T`, [принцип работы `key`, `load`, `save` описан тут](#shared-memory) |
...
| Метод                                        | Описание                                                                  |
|----------------------------------------------|---------------------------------------------------------------------------|
| bool empty() const | Проверить коллекцию на пустоту |
| size_t size() const | Получить текущее количество элементов в коллекции |
| size_t max_capacity() const | Получить максимальное количество элементов, которое можно разместить в коллекции |
| void clear() | Очистить коллекцию |
| void push_back(const T&amp; v) | Добавить элемент в конец коллекции, после текущего последнего элемента. Если добавлять некуда, выбрасывается `std::overflow_error` |
| void push_front(const T&amp; v) | Добавить элемент в начало коллекции, перед текущим первым элементом. Если добавлять некуда, выбрасывается `std::overflow_error` |
| void pop_back() | Удалить текущий последний элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| void pop_front() | Удалить текущий первый элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| const T&amp; back() const | Получить константную ссылку на текущий последний элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| T&amp; back() | Получить ссылку на текущий последний элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| const T&amp; front() const | Получить константную ссылку на текущий первый элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| T&amp; front() | Получить ссылку на текущий первый элемент коллекции. Если коллекция пуста, выбрасывается `std::underflow_error` |
| const T&amp; operator[](size_t i) const | Получить константную ссылку на `i`-ый элемент коллекции. Если `i` меньше нуля или больше `size()`, выбрасывается `std::out_of_range` |
| T&amp; operator[](size_t i) | Получить ссылку на `i`-ый элемент коллекции. Если `i` меньше нуля или больше `size()`, выбрасывается `std::out_of_range` |

## 10.11. Примеры доступа к параметрам портфеля, инструмента, сделки, позиций
...
<a id="__sec_status_check__"></a>Чтобы проверить, например, что в данный момент по инструменту можно выставлять заявки и в роботе есть активное подключение, получающее стаканы:
...
<a id="__Example1__"></a>Чтобы получить, например, цену последней сделки для финансового инструмента с СПб биржи (получение цены последней сделки по ключу возможно только для СПБ биржи):
...
<a id="__Example2__"></a>Чтобы, например, задать и потом получить значение "пользовательского поля#0":
...
<a id="__Example8__"></a>Чтобы найти, например, средневзвешенный бид по первым 10-ти элементам стакана (настоятельно НЕ рекомендуется искать что-то по всему стакану, т.к. стакан меняется часто, то работать ваш код будет долго):
...
<a id="__Example3__"></a>Чтобы пройти в цикле по всем финансовым инструмента портфеля и вывести имена финансовых инструментов в лог:
...
<a id="__Example9__"></a>Чтобы найти, например, сумму объемов активных заявок портфеля по финансовому инструменту по заданной цене 100:
...
## 10.12. Примеры использования функций и классов

Пусть имеется портфель с именем "test" и в этом портфеле есть два финансовых инструмента: фьючерс на индекс РТС "RIH6" и его опцион `call` на страйк 70000 "RI70000BB6", позиция по обоим финансовым инструментам в портфеле равна 1, а направление торговли у обоих [On buy](params-description.md#s.on_buy) = `Buy`.
...
где $i$ - `i`-тый инструмент портфеля, $\Delta_i$ - дельта `i`-го инструмента портфеля, $pos_i$ - позиция `i`-го инструмента портфеля, $On\enspace buy_i$ - [On buy](params-description.md#s.on_buy) `i`-го инструмента портфеля. Величины гамма, вега, тета и ожидаемая волатильность для портфеля рассчитываются аналогично. Функции для их расчета вызываются аналогично примеру с дельтой. Функция price также вызывается аналогично delta, но НЕ может быть вызвана для портфеля, может быть вызвана только для финансового инструмента.
...
<a id="__Example4__"></a>Чтобы проверить, что текущее время меньше полудня:
...
<a id="__Example7__"></a>Пример тaймера на каждые 10 секунд:
...
Пример получения и изменения "пользовательских полей" (увеличить значение `i`-го поля на `i`):<a id="__Example6__"></a>
...
## 10.13. Примеры написания Ratio buy/sell formula
...
Пусть имеется портфель с именем "si" и в этом портфеле есть один инструмент - фьючерс на доллар "SiH6", направление торговли этого инструмента [On buy](params-description.md#s.on_buy) = `Buy`.
...
<a id="__Example5__"></a>
## 10.14. Примеры использования индикаторов
...
## 10.15. Примеры работы с индикаторами и коллекциями, размещенными в `shared memory`
...
Вывести в лог список текущих индикаторов и коллекций робота, размещенных в `shared memory`:
```C
...
---
Создать дек ([`ring_deque`](c-api.md#ring-deque)) в `shared memory`, загрузив данные из `shared memory`, в качестве элемента дека использовать заданную структуру:
```C
struct point
{
  int x = 0;
...
Удалить заданную коллекцию, размещенную в `shared memory`:
```C
return indicators::del_indicator("okex", "rq8", "vec");
```

<a id="__shift_formula__"></a>
## 10.16. Примеры написания [Shift formula](params-description.md#p.shift_formula)
...
## 10.17. Наиболее частые ошибки в формулах, приводящие к аварийному завершению работы робота
