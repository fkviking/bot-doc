...
- В C++ интерфейс добавлен объект [`ring_deque`, который умеет сохранять и загружать свои значения при перезапуске робота](c-api.md#ring-deque)
- В связи с добавлением в C++ интерфейс коллекций, размещаемых в `shared_memory`, раздел `Индикаторы и математическая статистика` переименован в [`Индикаторы, математическая статистика и коллекции`](c-api.md#indicators-docs)
...
- В C++ интерфейс добавлена [возможность сохранения и загрузки значений индикаторов при перезапуске робота](c-api.md#sokhranenie-znachenii-indikatorov-mezhdu-perezapuskami-robota)
