...
Флаг признака игнорирования динамических лимитов по заявке. Если выбрать этот флаг, он будет проставлен у всех заявок, выставляемых с данного подключения. Необходимо уточнять на бирже, доступен ли вам этот флаг и в каких случаях его следует выставлять.

#### 6.4.2.11. Bind IP

IP-адрес, с которого будет происходить подключение к бирже: 0.0.0.0.
...
Следует [обратиться в поддержку](mailto:help@fkviking.ru) для уточнения bind IP. IP-адрес, указанный в этом поле, не следует предоставлять бирже, как IP-адрес сервера.
...
### 6.10.7. Описание потоков подключений BINANCEFUT
...
### 6.11.7. Описание потоков подключений BINANCECM
...
#### 6.14.2.1. Поля для подключения FIX

FIX подключение требует создания VPN-соединения, поэтому, перед добавлением подключения [обратитесь в поддержку](mailto:help@fkviking.ru) и предоставьте параметры VPN-соединения.

##### 6.14.2.1.1. Server

Выбор сервера для подключения. Если нужного адреса нет в списке, [обратитесь в поддержку](mailto:help@fkviking.ru).
...
#### 6.14.2.2. Поля для подключения WEBSOCKET
...
Выбор типа подключения. Если никаких договоренностей с биржей по поводу прямого подключения нет, то стоит выбрать тип REGULAR. Если есть договоренности с биржей по прямому подключению, то предварительно нужно [обратиться в поддержку](mailto:help@fkviking.ru) и узнать IP-адрес сервера, с которого будет осуществляться торговля, после этого при создании подключения стоит выбрать тип DIRECT.
...
Подключение в стадии разработки.
...
### 6.25.1. Рыночные данные (бинарный протокол)
...
### 6.25.2. Транзакционный шлюз бинарного протокола
...
В связи с особенностями данного подключения полная настройка возможна только после [обращения в техническую поддержку](mailto:help@fkviking.ru).
...
Идентификатор приложения-источника запроса. Это поле следует оставить пустым, если от брокера не поступало другой информации.

### 6.31.4. Token

Сгенерировать токен можно в личном кабинете Финам API. Тип доступа должен быть для торговли. При создании токена следует указывать только один счёт. Если нужно торговать с нескольких счетов, то надо создать несколько токенов, каждый со своим счетом, и, соответственно, добавить в робота несколько торговых подключений.
