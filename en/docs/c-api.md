...
Formulas in C++ can be edited in the `Formulas` tab of the portfolio settings window, opened by clicking the <img src="/bot-doc/docs/00-img/icons/settings_black.svg" width="16" height="16"/> icon.
...
## 10.2. Basic Principles of Writing Formulas
...
- The maximum length for each formula field in a portfolio is `12000` bytes (this value can be checked in the [portfolio template](api.md#get_template_id)).  
  ...
  If space is still insufficient, keep in mind that formulas are intended for small algorithm extensions. For more advanced customization, use the [WebSocket API](api.md#api).
...
## 10.3. Recommendations for Filtering Market Data
...
## 10.4. Basic Principles of Formula Debugging
...
## 10.5. Accessing Market Data for Financial Instruments
...
| double bid()<a id="bid-offer-check"></a>                    | Best bid price (`0` if unavailable; recommended to check together with `amount_bid`). |
...
| int trading_status()<a id="trading-status-check"></a>            | Trading status of the instrument on the exchange (bitmask, possible flags: [TRADING_CAN_PLACE](#__trading_can_place__), [TRADING_CAN_CANCEL](#__trading_can_cancel__), [example](#__sec_status_check__)). |
| int conn_online()<a id="market-data-check"></a>               | Market data connection status in the robot (bitmask, possible flags: [MARKET_DATA_BESTS_ONLINE](#__market_data_bests_online__), [MARKET_DATA_OB_ONLINE](#__market_data_ob_online__), [example](#__sec_status_check__)). |
...
| Method                 | Description                                                                                                                                                         |
|------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| bool is_available()<a id="order-book-check"></a> | Checks whether the order book for the instrument is currently available.                                                                                  |
...
## 10.6. Accessing and Modifying Portfolio Instrument Fields
...
## 10.7. Accessing and Modifying Portfolio Fields
...
| deal_item deal() <a id="__last_deal__"></a>  | get last trade for portfolio financial instruments, if no trades occurred, then `amount` and `price` will be zero |
...
To access "user fields" by index, arrays are available: <a id="user-fields"></a>
```C
...
## 10.8. Accessing and Modifying Positions of a Transaction Connection
...
## 10.9. Additional Classes, Structures, Functions, and Constants
...
| <a id="__trading_can_place__"></a>TRADING_CAN_PLACE  | int | 1, order placement allowed.                       |
| <a id="__trading_can_cancel__"></a>TRADING_CAN_CANCEL | int | 2, order cancellation allowed.                   |
| MARKET_DATA_OFFLINE   | int   | 0, market data connection for the instrument is offline.                          |
| <a id="__market_data_bests_online__"></a>MARKET_DATA_BESTS_ONLINE | int | 1, market data connection with best bid/ask is online. |
| <a id="__market_data_ob_online__"></a>MARKET_DATA_OB_ONLINE       | int | 2, market data connection with order books is online. |
| NAME           | std::string | Name of the current portfolio.                                                     |
| <a id="__null_value__"></a>NULL_VALUE     | long long   | Empty value for numeric fields, equal to `-(1 << 53)`. |
...
| void log_info(const std::string& msg) <a id="cpp-debug-functions"></a>            | Sends a log message with level INFO.                                        |
...
| void log_show(const std::string& msg) <a id="cpp-log-show"></a>                      | Send a message with the INFO level to the log; this message will be displayed in the [`Robot logs`] widget (interface.md#robot_logs)      |
| <a id="__tgr_notify__"></a> bool tgr_notify(int slot, const std::string& msg, int timeout) | Sends a log message with level NOTIFICATION and a Telegram alert. Messages must not be sent more frequently than once per `timeout` seconds (`[10, 2000000000]`). The `slot` parameter is the notification slot ID (`[0, 4]`) for which the timeout will be applied. |

### 10.9.3. Structures

The `day_time` structure (time of day in hours, minutes, seconds): <a id="day_time"></a>
...
Class `timer` (measures time intervals not shorter than the specified nanoseconds): <a id="timers"></a>
...
### 10.9.5. Option Functions

#### 10.9.5.1. _“Smart” Functions_
...
## 10.10. Indicators, Mathematical Statistics, and Collections

### 10.10.1. General Provisions

#### 10.10.1.1. Specifics of Objects Used for Indicator Calculation
...
#### 10.10.1.2. Saving Indicator Values and Collections Between Robot Restarts
...
##### 10.10.1.2.1. Implementation Details
...
##### 10.10.1.2.3. Viewing and Deleting Indicator and Collection Values Placed in `shared memory`
...
#### 10.10.1.3. `TradingDays` and `schedule` for Indicators
...
#### 10.10.1.4. Object `indicators::interval`
...
#### 10.10.1.5. Object `indicators::schedule`
...
#### 10.10.2.1. Simple Moving Average (`SMA`)
...
#### 10.10.2.2. Exponential Moving Average (`EMA`)
...
#### 10.10.2.3. Maximum over Interval (`Max`)
...
#### 10.10.2.4. Minimum over Interval (`Min`)
...
#### 10.10.2.6. Standard Deviation (`StdDev`)
...
#### 10.10.2.8. Relative Strength Index (`RSI`)
...
#### 10.10.3.1. Ring Buffer-Based Deque (`ring_deque`)
...
## 10.11. Examples of Accessing Portfolio, Instrument, Trade, and Position Parameters
...
<a id="__sec_status_check__"></a>To check, for example, whether orders can currently be placed for the instrument and whether the robot has an active connection receiving order books:
...
<a id="__Example1__"></a>To get, for example, the last trade price for a financial instrument from the SPB exchange (retrieving the last trade price by key is only available for the SPB exchange):
...
<a id="__Example2__"></a>To, for example, set and then get the value of "user field #0"
...
<a id="__Example8__"></a>To calculate, for example, the weighted average bid over the first 10 order book entries (strongly NOT recommended to iterate over the entire order book, as it changes frequently and your code will run slowly):
...
<a id="__Example3__"></a>To loop through all financial instruments in the portfolio and output their names to the log:
...
<a id="__Example9__"></a>To calculate, for example, the total volume of active portfolio orders for a financial instrument at a given price of 100:
...
## 10.12. Examples of Using Functions and Classes
...
<a id="__Example4__"></a>To check if the current time is before noon:
...
<a id="__Example7__"></a>Example of a timer every 10 seconds:
...
Example of accessing and modifying "user fields" (increase the value of the `i`-th field by `i`):<a id="__Example6__"></a>
...
## 10.13. Examples of Writing Ratio Buy/Sell Formulas
...
<a id="__Example5__"></a>
## 10.14. Examples of Using Indicators
...
<a id="__ExampleInd__"></a>
## 10.15. Examples of Working with Indicators and Collections Placed in `shared memory`
...
---
Create a deque ([`ring_deque`](c-api.md#ring-deque)) in `shared memory`, loading data from `shared memory`, using a given struct as the deque element:
```C
struct point
{
  int x = 0;
...
<a id="__shift_formula__"></a>
## 10.16. [Shift formula](params-description.md#p.shift_formula) examples
...
## 10.17. Most Common Errors in Formulas Leading to Robot Crash
