...
![Doc](/bot-doc/docs/00-img/bot-scheme-en.svg)
...
- All necessary market data connections are configured in them. Changing state of market data connections (`Enabled`/`Disabled`) is prohibited.
- Creating trading connections is prohibited, because such robots are not intended for live trading. During initial setup of such robots, [Connection setup](getting-started.md#connection_properties) section should be skipped.
...
`Head of traders` can assign `Trader`, `Head of traders`, and [Head view only](introduction.md#head_view_only_role) roles to themselves and other users within a company where they are `Head of traders`. In particular, they can grant access to robots and portfolios to users who have `Trader` role in a company. Users with `Head of traders` and [Head view only](introduction.md#head_view_only_role) roles do not need to choose available robots and portfolios, they have access to all robots and portfolios within a company by default. Access is granted in [Users](interface.md#widget_users) widget, available only to users with `head of traders`/[Head view only](introduction.md#head_view_only_role) roles.

In addition, a user with `Head of traders` role gets a portfolio parameter from Anti "Spam" group called `max_not_hedged_adm`. This parameter sets maximum possible value of [max_not_hedged](params-description.md#p.max_not_hedged) parameter. Restriction applies to a portfolio on which restriction is set.

### 2.5.3. Head view only

`Head view only` is a read-only counterpart of [Head of traders](introduction.md#head_of_traders_role) role, under this role same data is visible as under [Head of traders](introduction.md#head_of_traders_role), but modification of this data is prohibited. Under `Head view only` role, only own user settings can be edited.
