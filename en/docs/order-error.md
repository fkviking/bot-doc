...
- **Error:** `Order adding error on *, error: REASON_NO_MONEY`<a id="error.no_money"></a>
...
- **Error:** `order adding error on *, continue trading, error: REASON_FLOOD`<a id="error.flood"></a>
...
- **Error:** `order adding error on *, error: REASON_UNDEFINED`<a id="error.undefined"></a>
...
- **Error:** `order adding error on *, continue trading, error: REASON_PRICE_WRONG`<a id="error.price_wrong"></a>
...
- **Error:** `order adding error on *, continue trading, error: REASON_CROSS`<a id="error.cross"></a>
...
- **Error:** `order adding error on *, continue trading, error: REASON_ZERO_AMOUNT_TO_MULTIPLE`<a id="error.zero_amount_to_multiple"></a>

    **Description:** On certain exchanges (e.g., Deribit), the order size must be a multiple of the lot size. Before submitting an order, the size is rounded down to the nearest whole number of lots. If the intended order size is smaller than one lot, the rounded size becomes zero. The robot detects this and returns the error. To avoid such errors, you can set the [Overlay](/bot-doc/en/docs/params-description.md#p.overlay) parameter to one or higher. This error only occurs with the `LOCAL` postfix.
    
- **Error:** `order adding error on *, continue trading, error: REASON_NO_SESSION`<a id="error.no_session"></a>
...
- **Error:** `order adding error on *, error: REASON_TEMPORARY_PROHIBITED_LOCAL, original err_code=666666`<a id="error.temporary_prohibited"></a>
